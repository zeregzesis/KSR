...
public class ConfusionMatrix {
    ...
    public double calculateAccuracy() {
        ...
        if(sumOfAllItems==0.0) return 0;
        else return allTPs/sumOfAllItems;
    }
    ...
    public double calculatePrecisionForClassification(){
        ...
        for ( Places place: Places.values()){
            nominator+=calculatePrecisionForCountry(place)*((getFN(place)+getTP(place))/numberOfAllArticlesClassified());
        }
        return nominator/6;
    }
    ...
    public double calculateRecallForClassification(){
        ...
        double nominator=0.0;
        for ( Places place: Places.values()){
            nominator+=calculateRecallForCountry(place)*((getFN(place)+getTP(place))/numberOfAllArticlesClassified());
        }
        return nominator/6;
    }
    ...
    public double calculateF1scoreForClassification(){
        ...
        double nominator=0.0;
        for ( Places place: Places.values()){
            nominator+=calculateF1ScoreForCountry(place)*((getFN(place)+getTP(place))/numberOfAllArticlesClassified());
        }
        return nominator/6;
    }
